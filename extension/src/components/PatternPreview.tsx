...
function PreviewCard({
  ...
  return (
    <div
      className={`${cardBase} ${strong ? "border-cyan-300/70 bg-cyan-300/20" : "border-slate-500/35 bg-slate-700/50"} ${className}`}
    >
      {children ?? (
        <>
          <div className="h-1.5 w-1/2 rounded-full bg-white/45" />
          <div className="mt-2 h-1 w-3/4 rounded-full bg-white/20" />
          <div className="mt-1 h-1 w-2/5 rounded-full bg-white/15" />
        </>
      )}
    </div>
  );
      ...
      <div className={frameClass}>
        <div className="grid h-full grid-cols-[0.9fr_1.1fr] items-center gap-2">
          <div className="space-y-2">
            ...
            <div className="h-3 w-12 rounded bg-cyan-200/70" />
          </div>
    ...
    return (
      <div className={frameClass}>
        <div className="flex h-full flex-col gap-2">
          <div className="mx-auto h-2 w-24 rounded-full bg-white/45" />
    ...
    return (
      <div className={frameClass}>
        <div className="flex h-full flex-col justify-between gap-2">
          <div className="mx-auto mt-1 h-2 w-24 rounded-full bg-white/50" />
          <div className="mx-auto h-3 w-14 rounded bg-cyan-200/70" />
          <div className="grid grid-cols-4 gap-1.5">
            {[1, 2, 3, 4].map((item) => (
              <div className="h-5 rounded border border-slate-500/35 bg-slate-700/50" key={item} />
            ))}
          </div>
        </div>
      </div>
        ...
        <div className="grid h-full grid-cols-3 items-center gap-2">
          {[1, 2, 3].map((step) => (
            <div className="relative h-full" key={step}>
              {step < 3 ? (
                <div className="absolute left-[72%] top-4 h-px w-7 bg-cyan-300/50" />
              ) : null}
              <div className="mb-1 flex h-5 w-5 items-center justify-center rounded-full bg-cyan-300/25 text-[10px] font-bold text-cyan-100">
                {step}
              </div>
    ...
    return (
      <div className={frameClass}>
        <div className="grid h-full grid-cols-3 items-center gap-2">
          ...
          <PreviewCard strong className="h-20 p-2 shadow-lg shadow-cyan-950/50" />
          <PreviewCard className="h-16 p-2 opacity-75" />
        </div>
      </div>
    );
    ...
    return (
      <div className={frameClass}>
        <div className="grid h-full grid-cols-3 items-end gap-2">
          ...
          <div className="h-full rounded-md border border-cyan-300/70 bg-cyan-300/20 p-2 shadow-lg shadow-cyan-950/50">
            <div className="mb-1 h-2 w-10 rounded-full bg-cyan-200/80" />
            <div className="h-1.5 w-1/2 rounded-full bg-white/55" />
            <div className="mt-2 h-1 w-4/5 rounded-full bg-white/25" />
            <div className="mt-3 h-3 rounded bg-cyan-200/70" />
          </div>
          <PreviewCard className="h-[4.2rem] p-2" />
        </div>
      </div>
        ...
        <div className="grid h-full grid-cols-[0.9fr_1fr_1fr_1fr] gap-1.5">
          {[0, 1, 2, 3].map((column) => (
            <div className="space-y-1.5" key={column}>
              <div className={column === 2 ? "h-4 rounded bg-cyan-300/35" : "h-4 rounded bg-slate-700/60"} />
      ...
      <div className={frameClass}>
        <div className="grid h-full grid-cols-[1.15fr_0.85fr] gap-2">
          <PreviewCard strong className="flex flex-col justify-center p-3">
            <div className="h-3 w-12 rounded-full bg-cyan-100/80" />
    ...
    return (
      <div className={frameClass}>
        <div className="grid h-full grid-cols-[0.8fr_1.1fr] gap-2">
          <div className="space-y-2 rounded-md border border-slate-500/35 bg-slate-700/45 p-2">
            <div className="h-1.5 w-14 rounded-full bg-white/45" />
            <div className="h-1 w-12 rounded-full bg-white/20" />
            <div className="h-1 w-16 rounded-full bg-white/20" />
          </div>
          <div className="space-y-1.5 rounded-md border border-cyan-300/50 bg-cyan-300/10 p-2">
            <div className="h-2 rounded bg-white/20" />
            <div className="h-2 rounded bg-white/20" />
            <div className="h-2 rounded bg-white/20" />
            <div className="mt-2 h-3 rounded bg-cyan-200/70" />
          </div>
        </div>
      </div>
    );
...
function GenericCategoryPreview({ category }: { category: PatternCategory }) {
  if (category === "hero" || category === "cta") {
    return (
      <div className="flex h-full flex-col justify-center gap-2">
        ...
        <div className="h-3 w-14 rounded bg-cyan-200/65" />
      </div>
    );
  }
  ...
  if (category === "form") {
    return (
      <div className="space-y-2 rounded-md border border-cyan-300/40 bg-cyan-300/10 p-2">
        <div className="h-2 rounded bg-white/20" />
        <div className="h-2 rounded bg-white/20" />
        <div className="h-2 rounded bg-white/20" />
        <div className="h-3 rounded bg-cyan-200/70" />
      </div>
    );
  }
